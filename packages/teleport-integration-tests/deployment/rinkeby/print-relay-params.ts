...
async function printRelayParams(l1Provider: Provider, l2Provider: Provider) {
  ...
  const maxSubmissionCost = await getMaxSubmissionPrice(l1Provider, calldataLength, rinkebySdk.arbitrum.inbox.address)
  const maxGas = await getMaxGas(l2Provider, L1_GOV_RELAY_ADDR, L2_GOV_RELAY_ADDR, L2_GOV_RELAY_ADDR, l2MessageCalldata)
