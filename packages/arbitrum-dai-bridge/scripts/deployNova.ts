...
import { deployBridge, getNovaNetworkConfig, getNovaRouterDeployment, performSanityChecks } from '../arbitrum-helpers'

async function main() {
  const pkey = getRequiredEnv('L1_MAINNET_DEPLOYER_PRIV_KEY')
  const l1Rpc = getRequiredEnv('L1_MAINNET_RPC_URL')
  const l2Rpc = getRequiredEnv('L2_NOVA_RPC_URL')

  const network = await getNovaNetworkConfig({ pkey, l1Rpc, l2Rpc })
  console.log(`Deploying to mainnet Ethereum/Nova using: ${network.l1.deployer.address}`)
  const routerDeployment = await getNovaRouterDeployment(network)
