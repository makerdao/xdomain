...
export async function fundTestWallet(
  ...
  const l2Provider = l2Wallet.provider || l2Sdk.WormholeOutboundGateway!.provider
  const l1Provider = l1Wallet.provider || l1Sdk.WormholeJoin!.provider
  ...
  const l2DaiLike = new Contract(
    await l2Sdk.WormholeOutboundGateway!.l2Token(),
